...
def make_dots(walls) -> list:
    ...
    for x in range(0, 6 * d.GRID + 1, d.GRID):
        for y in range(12 * d.GRID, 22 * d.GRID + 1, d.GRID):
    ...
    for x in range(8 * d.GRID, 21 * d.GRID + 1, d.GRID):
        for y in range(12 * d.GRID, 22 * d.GRID + 1, d.GRID):
            where_dots_cant_be.append((x, y))
    where_dots_cant_be.extend([(2 * d.GRID, 6 * d.GRID), (27 * d.GRID, 6 * d.GRID),
                               (2 * d.GRID, 26 * d.GRID), (27 * d.GRID, 26 * d.GRID)])

    dots = []
    for x in range(d.GRID * 2, d.WIDTH - d.GRID, d.GRID):
        for y in range(d.GRID * 4, d.HEIGHT - 2 * d.GRID, d.GRID):
